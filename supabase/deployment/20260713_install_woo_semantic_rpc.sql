-- Installation script for exec_dashboard.get_woo_metrics_semantic (PASS 2B.1B)
-- Recommended environment: psql (with \set ON_ERROR_STOP on). Remove the meta-command if using Supabase SQL Editor.
...
BEGIN
  IF to_regprocedure('exec_dashboard.get_woo_metrics_semantic(date,date)') IS NOT NULL THEN
    RAISE EXCEPTION 'Installation aborted: exec_dashboard.get_woo_metrics_semantic(date,date) already exists';
  END IF;
END
$precheck$;
...
BEGIN
  fn_oid := 'exec_dashboard.get_woo_metrics_semantic(date,date)'::regprocedure;
  IF fn_oid IS NULL THEN
    RAISE EXCEPTION 'Post-install: function not found';
  END IF;

  SELECT n.nspname AS schema_name, p.proname, pg_get_userbyid(p.proowner) AS owner,
         p.provolatile, p.prosecdef, l.lanname, p.proconfig
  INTO fn_record
  FROM pg_proc p
  ...
  IF fn_record.schema_name <> 'exec_dashboard' THEN
    RAISE EXCEPTION 'Post-install: schema mismatch (%)', fn_record.schema_name;
  END IF;
  IF fn_record.proname <> 'get_woo_metrics_semantic' THEN
    RAISE EXCEPTION 'Post-install: proname mismatch (%)', fn_record.proname;
  END IF;
  IF fn_record.owner <> 'postgres' THEN
    RAISE EXCEPTION 'Post-install: owner mismatch (%)', fn_record.owner;
  END IF;
  IF fn_record.provolatile <> 's' THEN
    RAISE EXCEPTION 'Post-install: volatility mismatch (%)', fn_record.provolatile;
  END IF;
  IF NOT fn_record.prosecdef THEN
    RAISE EXCEPTION 'Post-install: SECURITY DEFINER not set';
  END IF;
  IF fn_record.lanname <> 'plpgsql' THEN
    RAISE EXCEPTION 'Post-install: language mismatch (%)', fn_record.lanname;
  END IF;

  cfg := fn_record.proconfig;
  IF cfg IS NULL OR array_position(cfg, 'search_path=public, exec_dashboard') IS NULL THEN
    RAISE EXCEPTION 'Post-install: search_path config missing (%)', cfg;
  END IF;
  ...
  IF granted_anon THEN
    RAISE EXCEPTION 'Post-install: anon unexpectedly has EXECUTE';
  END IF;
  ...
  IF NOT has_schema_privilege('service_role', 'exec_dashboard', 'USAGE') THEN
    RAISE EXCEPTION 'Post-install: service_role lacks exec_dashboard schema usage';
  END IF;
END
$assert$;
