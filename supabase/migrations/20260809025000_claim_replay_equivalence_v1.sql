-- claim_replay_equivalence_v1
--
-- Purpose:
-- Allow idempotent replay of a Claim version when the only drift is the
-- observation metadata field `payload_json.retrieved_at`.
--
-- Constraints:
-- - Do NOT rewrite existing Claim versions or recompute stored content_hash.
-- - Do NOT change semantic uniqueness constraint external_claim_versions_v1__fingerprint_policy_uniq.
-- - Preserve legitimate new-version behavior when fingerprint/policy changes.
