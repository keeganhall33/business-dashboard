-- External Intelligence EvidenceReference persistence: ensure parent row exists before inserting version.
--
-- Root cause:
-- - external_evidence_reference_versions_v1 has an FK to external_evidence_references_v1.
-- - persist_external_evidence_reference_v1 inserted the version row BEFORE inserting the parent row,
--   causing FK violation for new references.
--
-- Scope:
-- - Change ONLY the insert order in public.persist_external_evidence_reference_v1 (new-reference path).
-- - Preserve: signature, SECURITY DEFINER, search_path, auth gate, V2 fingerprint behavior, idempotency, validation.
