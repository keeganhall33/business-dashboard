-- Fix: persist_external_claim_v1 must ensure parent external_claims_v1 exists before inserting a claim version.
--
-- Production error:
-- - FK violation external_claim_versions_v1_claim_id_fkey
--
-- Root cause:
-- - persist_external_claim_v1 inserted into external_claim_versions_v1 (child) before inserting into external_claims_v1 (parent).
-- - external_claims_v1 has NOT NULL fields (claim_id, current_content_hash, interpretation_policy_version).
--
-- Contract:
-- - external_claims_v1.current_content_hash == latest claim-version content_hash for that claim_id.
--
-- Scope (minimal):
-- - Change ONLY the parent creation order + required parent fields.
-- - Preserve: signature, SECURITY DEFINER, search_path, auth gate (auth.jwt role), V2 hash checks,
...
begin
  ...
  if found then
    ...
    -- Fail closed if caller tries to mutate interpretation policy identity.
