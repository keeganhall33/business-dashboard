...
-- Phase B4: Atomic recurring internal orchestration activation.
--
-- This migration provides narrowly scoped transactional RPCs that atomically:
-- - upsert the canonical scheduled heartbeat row (scheduled_jobs)
-- - enable/disable exactly the four approved internal orchestration jobs
-- - enforce external-collection safety gates
-- - persist non-secret activation/disable audit records (system_state)
--
-- IMPORTANT:
-- - This does NOT create a cron trigger. Production recurrence depends on an external runner
--   invoking /api/scheduler/tick (not represented in-repo).
-- - These RPCs are service-role only.

create extension if not exists pgcrypto;

-- Dependencies from the core scheduler tables. These exist in production, but the disposable
-- Postgres validation path applies only the external-intelligence migrations, so we defensively
-- create the minimal required tables if missing.
...
-- Internal constants (keep them explicit and fail-closed).
do $$
begin
  -- No-op block: placeholder for future validation asserts.
end $$;
...
declare
  ...
  -- scheduled_jobs.route_path is metadata only; dispatch is by job_key inside the tick runner map.
...
begin
  ...
  -- Activation ID must be create-only.
  ...
  -- Safety gates: no active heartbeat lease.
  ...
  -- Safety gates: external collection must remain fully disabled.
  ...
  -- Unknown enabled internal jobs are forbidden.
  ...
  -- Read current scheduled heartbeat row if present.
  select * into v_existing_sched
  from public.scheduled_jobs
  where job_key = v_job_key;

  -- Snapshot pre-state (safe). Keep it small.
  ...
  -- Determine current active configuration hash (if any).
  ...
  -- Validate internal job definitions: rows must exist or be safely upserted.
  -- We preserve next_run_at/last_* history; we only set config pins + enabled state.

  -- Pre-compute boundaries for deterministic initialization.
  ...
  -- Ensure next_run_at exists for the approved jobs (do not force them due immediately).
  ...
  -- Upsert scheduled heartbeat row; preserve an existing future next_run_at.
  select * into v_existing_sched
  from public.scheduled_jobs
  where job_key = v_job_key
  for update;
  ...
  v_sched_next := null;
  if v_existing_sched.next_run_at is not null and v_existing_sched.next_run_at > v_now then
  ...
  -- Enable exactly the four approved internal jobs.
  update public.internal_orchestration_jobs_v1
    set enabled = true,
        updated_at = v_now
  where environment = 'production'
  ...
  -- Postcondition: exactly four enabled jobs.
  ...
  -- Postcondition: scheduled job row exists and is active with pinned cadence.
  if not exists (
  ...
  -- Persist canonical state (for idempotency + conflict detection).
  insert into public.system_state(key, value_json, updated_at)
  values (
    v_state_key,
    jsonb_build_object(
...
begin
  ...
  -- Determine whether we were already inactive (for idempotent disable replays).
  select value_json into v_state
  from public.system_state
  where key = v_state_key;
  if v_state is not null then
  ...
  -- Disable canonical heartbeat scheduled job (if present).
  update public.scheduled_jobs
    set is_active = false,
        updated_at = v_now
  where job_key = v_job_key;

  -- Disable exactly the four approved internal jobs.
  update public.internal_orchestration_jobs_v1
    set enabled = false,
        updated_at = v_now
  where environment = 'production'
