-- External Intelligence persistence RPC authz (SECURITY DEFINER-safe): authorize via auth.jwt() claims.
--
-- Root cause:
-- - persist_external_evidence_reference_v1 / persist_external_claim_v1 are SECURITY DEFINER.
-- - Inside SECURITY DEFINER, current_user/current_role reflect the definer (e.g. postgres), not the caller.
-- - PostgREST request role is caller context; caller identity must be derived from request JWT claims.
--
-- Scope (minimal):
-- - Change ONLY the service_role authorization gate in:
--   * public.persist_external_evidence_reference_v1
--   * public.persist_external_claim_v1
--   to use: coalesce(auth.jwt()->>'role','') = 'service_role'
-- - Add a NO-WRITE SECURITY DEFINER diagnostic RPC to prove the above from inside definer context.
-- - Preserve: signatures, SECURITY DEFINER, owner, search_path, grants, validations, idempotency, V2 behavior.

-- 1) Diagnostic SECURITY DEFINER RPC (no writes, no table reads)
create or replace function public.debug_request_jwt_role_definer_v1()
returns jsonb
language plpgsql
stable
security definer
...
begin
  v_auth := auth.jwt();
  v_role := nullif((v_auth ->> 'role'), '');
  v_ref := nullif((v_auth ->> 'ref'), '');
  ...
  return jsonb_build_object(
    ...
    'auth_jwt_role', coalesce(v_role, '<blank>'),
    'auth_jwt_ref',  coalesce(v_ref, '<blank>')
  );
end;
$$;

revoke all on function public.debug_request_jwt_role_definer_v1() from public;
grant execute on function public.debug_request_jwt_role_definer_v1() to anon, authenticated, service_role;
...
-- Preserve existing EXECUTE grants surface (re-assert explicitly).
