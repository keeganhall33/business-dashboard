-- External Intelligence persistence RPC authz: authorize via PostgREST request role impersonation.
--
-- Root cause:
-- - PostgREST impersonates the request role (SET LOCAL ROLE), exposed as current_user/current_role/current_setting('role').
-- - session_user remains the underlying authenticator and must not be used for request-role authorization.
--
-- Scope:
-- - Change ONLY the service-role authorization gate in:
--   * public.persist_external_evidence_reference_v1
--   * public.persist_external_claim_v1
-- - Preserve:
--   signatures, V2 behavior, SECURITY DEFINER, search_path, grants, idempotency, validations.

-- EvidenceReference persistence RPC: gate via current_user.
...
declare
  ...
  v_role text;
begin
  -- Authorization: service_role request role only.
...
-- Claim persistence RPC: gate via current_user.
