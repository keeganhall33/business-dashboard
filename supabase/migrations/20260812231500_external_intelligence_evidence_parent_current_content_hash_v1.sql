-- Fix: persist_external_evidence_reference_v1 must populate parent external_evidence_references_v1.current_content_hash.
--
-- Root cause:
-- - external_evidence_references_v1 has NOT NULL fields:
--   * evidence_reference_id
--   * current_content_hash
--   * source_id
--   * source_config_version
--   * legal_policy_version
-- - New-reference path inserted parent with only (evidence_reference_id), causing NOT NULL violation.
--
-- Contract (confirmed by historical rows):
-- - external_evidence_references_v1.current_content_hash == latest version content_hash for that evidence_reference_id.
--
-- Scope:
-- - Change ONLY parent upsert in public.persist_external_evidence_reference_v1.
-- - Preserve: SECURITY DEFINER, signature, auth gate (auth.jwt role), parent-before-version ordering,
--   V2 hash checks, grants, idempotency, validations.
...
begin
  ...
  if version_exists then
    ...
    -- Parent current_content_hash points at the latest version content_hash.
    ...
    on conflict on constraint external_evidence_references_v1_pkey
    do update set
      current_content_hash = excluded.current_content_hash,
      updated_at = now();

    -- Fail closed if caller tries to mutate source identity fields on an existing reference.
    select * into parent_row
    from public.external_evidence_references_v1 er
    where er.evidence_reference_id = in_evidence_reference_id;

    if parent_row.source_id is distinct from in_source_id
      or parent_row.source_config_version is distinct from in_source_config_version
      or parent_row.legal_policy_version is distinct from in_legal_policy_version then
      raise exception using errcode = 'P0001', message = 'integrity_conflict';
    end if;
