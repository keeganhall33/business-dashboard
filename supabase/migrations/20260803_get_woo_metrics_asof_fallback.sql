-- Slice #3 prerequisite: Woo selected-range telemetry freshness
--
-- Populate get_woo_metrics().summary.asOf using:
-- - first choice: woo_ingestion_runs_v1.source_as_of_gmt (provider/data as-of)
-- - fallback:    woo_ingestion_runs_v1.completed_at     (ingestion completion / retrieval time)
--
-- IMPORTANT: completed_at fallback is NOT proof the source data itself was current at that moment.
-- Future UI must use neutral wording (e.g. "Updated" / "Retrieved") unless source_as_of_gmt is present.
