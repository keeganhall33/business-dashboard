-- Fix persist_external_event_v1 again: avoid output-parameter name collisions via dynamic SQL.
...
begin
  ...
  execute
    'insert into public.external_events_v1 (event_id,current_content_hash,event_type,lifecycle_status,correction_status)
     values ($1,$2,$3,$4,coalesce($5, ''none''))
     on conflict (event_id) do nothing'
  using in_event_id, in_content_hash, in_event_type, in_lifecycle_status, in_correction_status;
  ...
  execute
    'update public.external_events_v1
     set current_content_hash = $2,
         event_type = $3,
         lifecycle_status = $4,
         correction_status = coalesce($5, ''none''),
         updated_at = now()
     where event_id = $1'
  using in_event_id, in_content_hash, in_event_type, in_lifecycle_status, in_correction_status;

  execute
    'insert into public.external_event_claim_links_v1 (link_id,event_id,event_content_hash,claim_id,claim_content_hash)
     values ($1,$2,$3,$4,$5)
     on conflict (event_id, event_content_hash, claim_id, claim_content_hash) do nothing'
  using in_link_id, in_event_id, in_content_hash, in_claim_id, in_claim_content_hash;
