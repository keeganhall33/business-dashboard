...
const wt = path.join(os.tmpdir(), 'orch-pr418-proof-integrity');
const run = (cmd, args, opts = {}) => execFileSync(cmd, args, { encoding: 'utf8', stdio: opts.stdio ?? 'inherit', cwd: opts.cwd });

try { run('git', ['worktree','remove','--force',wt], { stdio: 'ignore' }); } catch {}
run('git', ['fetch','origin',target]);
run('git', ['worktree','add','-B','agent/orch410-openclaw-json-extract-fix',wt,`origin/${target}`]);

const runnerPath = path.join(wt, 'scripts/orchestration-run-issue-openclaw.mjs');
let runner = fs.readFileSync(runnerPath, 'utf8');
runner = runner.replace('import { spawnSync } from "node:child_process";', 'import { execFileSync, spawnSync } from "node:child_process";');
