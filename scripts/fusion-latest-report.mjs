...
function hasColumns(table, cols) {
  const rows = query(
    `select column_name from information_schema.columns where table_schema='public' and table_name='${table}' and column_name in (${cols
      .map((c) => `'${c}'`)
      .join(",")}) order by column_name;`
  );
...
const hasStatus = hasColumns("fusion_runs_v1", statusCols);
...
const latest = query(
  hasStatus
    ...
    : "select run_id, generated_at, selected_candidate_id, review_by from fusion_runs_v1 order by generated_at desc limit 1;"
);
