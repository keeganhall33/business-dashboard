...
async function fetchWooEligibleOldCreatedWindow({ start, end }) {
  ...
  // OLD / INVALID FOR PAID-DATE PARITY:
  // uses after/before (created-date filter) + orderby=date.
  ...
  while (true) {
    ...
    for (const order of rows) {
      ...
      if (paidPacificDate < start || paidPacificDate > end) continue;
  ...
  return {
    query: { after, before, orderby: "date", order: "desc" },
    pagesRequested,
    pagesCompleted,
    retryCount,
    malformedRows,
    ...
    ids: rows.map((r) => r.id),
    currencySet: Array.from(new Set(rows.map((r) => r.currency))).sort(),
    totals: {
...
async function fetchTelemetryEligible({ start, end }) {
  ...
  const currencySet = Array.from(new Set(out.map((r) => String(r.currency ?? "").toUpperCase()))).sort();
  const totals = {
  ...
  return { count: ids.length, idSetHash: hashIdSet(ids), currencySet, totals };
}
...
async function main() {
  ...
  const woo = await fetchWooEligibleOldCreatedWindow({ start, end });
  ...
  process.stdout.write(JSON.stringify({ range: { start, end }, woo, telemetry, exactMatch }, null, 2) + "\n");
}
