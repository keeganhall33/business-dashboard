...
import { claimTask, getTask, recordSemanticProgress, releaseSlotForTerminalTask, transitionTask } from '../state-store/sqlite-store.mjs';
...
export async function runV4Task({
  ...
  try {
    ...
    transitionTask(db, {
      ...
      patch: { workspacePath: workspace.workspacePath },
      now: now(),
    });
    ...
    if (result.status === 'COMPLETE') {
      ...
      transitionTask(db, {
        ...
        patch: { terminalReason: result.reason ?? result.status ?? 'EXECUTION_FAILED' },
        now: now(),
      });
    }
    ...
    if (current && [V4_STATES.CLAIMED, V4_STATES.RUNNING, V4_STATES.VALIDATING].includes(current.state)) {
      try {
        transitionTask(db, {
          ...
          patch: { terminalReason: String(error?.message ?? error) },
          now: now(),
        });
      } catch {}
    }
    ...
    if (terminal && [V4_STATES.COMPLETE, V4_STATES.BLOCKED, V4_STATES.FAILED, V4_STATES.TIMED_OUT].includes(terminal.state)) {
