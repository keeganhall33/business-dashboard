...
const runtimePrompt = [
  ...
  'Use exec for pwd, git status, ls, find, directory inspection, and every other shell command.',
  ...
  'When a requested target file does not exist yet, create it with apply_patch or exec instead of repeatedly trying to read it.',
  'After any tool failure, inspect the failure and switch strategy. Do not repeat the same invalid read or path pattern.',
