...
const runtimePrompt = [
  ...
  '',
  '',
  'Concrete apply_patch directive rules:',
  '  - Add File: every content line MUST begin with + (e.g., *** Begin Patch',
  '    *** Add File: relative-or-absolute-path',
  '    + new line one',
  '    + new line two',
  '    *** End Patch)',
  '',
  '  - Update File: use @@ hunks with context and +/- lines;',
  '    never emit an empty Update File directive (no hunk headers with no content).',
  '    Example:',
  '      *** Begin Patch',
  '      *** Update File: relative-or-absolute-path',
  '      @@ -1,3 +1,4 @@',
  ...
  '      *** End Patch',
  '',
  '  - Delete File: use only the supported Delete File directive;',
  '    never emit Remove File or unsupported syntax.',
  '    Example:',
  '      *** Begin Patch',
  '      *** Delete File: relative-or-absolute-path',
  '      *** End Patch',
  '',
  '  - One path must NOT receive conflicting Add, Update, or Delete directives in one payload.',
  '',
  '  - *** End Patch MUST be the final line of the payload; no content may follow.',
  '',
  'Malformed payload examples and their failover behavior:',
  '',
  '  Example 1: Malformed Add File (raw unprefixed content instead of + prefixed lines)',
  '    *** Begin Patch',
  '    *** Add File: new-file.txt',
  '    raw line without prefix',
  '    *** End Patch',
  '    => Parser error. Failover: use shell exec cat > target-path <<\'EOF\' ... EOF.',
  '',
  '  Example 2: Missing End Patch terminator',
  '    *** Begin Patch',
  ...
  '     existing line two',
  '    *** End Patch',
  '    stray content after hunk',
  '    => Parser error. Failover: use shell exec.',
  '',
  '  Example 3: Conflicting directives for same path',
  '    *** Begin Patch',
  ...
  '    => Parser error. Failover: use shell exec.',
  '',
  '  Example 4: Empty Update File hunk',
  '    *** Begin Patch',
  '    *** Update File: file.txt',
  '    @@ -0,0 +0,0 @@',
  '    *** End Patch',
  '    => Not allowed. Failover: use shell exec.',
  '',
  '  Example 5: Unsupported Remove File syntax',
  '    *** Begin Patch',
  '    *** Remove File: old-file.txt',
  '    *** End Patch',
  '    => Parser error (Remove File unsupported). Failover: use shell exec or Delete File if deletion needed.',
  '',
  prompt,
].join('\n');
