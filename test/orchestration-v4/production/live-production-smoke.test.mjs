    ...
    const report = await runLiveProductionSmoke({
      ...
      poll: async ({ db }) => {
        insertReadyTask(db, {
          ...
          baseSha,
          contract: { title: 'Smoke fixture', body: 'fixture body', taskMutability: 'IMPLEMENTATION_MUTATION_REQUIRED', fileOwnership: 'tmp/v4-smoke.txt' },
        });
        db.prepare("UPDATE tasks SET state='COMPLETE', semantic_progress_seq=2, result_json=? WHERE task_id=?")
          .run(JSON.stringify({ publicationRequired: true, prNumber: 4321, prUrl: 'https://example.invalid/pr/4321', headSha: 'b'.repeat(40), branch: 'v4/issue-1234-smoke-1234' }), 'smoke-1234');
