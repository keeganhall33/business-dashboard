...
import { createSlotRegistry, candidateSlots, claimSlot, releaseSlot } from '../../scripts/orchestration-v4/slot-scheduler.mjs';

test('slots express capacity and routing only', () => {
  const registry = createSlotRegistry();
  assert.deepEqual(candidateSlots(registry, 'CORE_INTELLIGENCE'), ['local-a']);
...
test('claim and release are task-scoped and fail closed on cross-task release', () => {
  ...
  assert.deepEqual(candidateSlots(registry, 'CORE_INTELLIGENCE'), []);
