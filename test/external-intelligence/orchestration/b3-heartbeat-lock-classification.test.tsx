...
test("b3 heartbeat: acquired=true with token succeeds (no handlers enabled)", async () => {
  const overrides: HeartbeatOverrides = {
    withJobRun: async ({ fn }) => fn(),
    now: () => new Date("2026-08-05T00:00:00.000Z"),
    acquireLock: async () => ({ acquired: true, lease_token: "t", expires_at: "2026-08-05T00:05:00.000Z" }),
...
test("b3 heartbeat: empty RPC data fails closed as acquired=false", async () => {
  // Simulate malformed/empty RPC data case by returning acquired=false without a token.
  // (The rpc data parsing in lock.ts yields this shape when data is empty/null.)
  const overrides: HeartbeatOverrides = {
    withJobRun: async ({ fn }) => fn(),
    now: () => new Date("2026-08-05T00:00:00.000Z"),
    acquireLock: async () => ({ acquired: false, lease_token: null, expires_at: null }),
