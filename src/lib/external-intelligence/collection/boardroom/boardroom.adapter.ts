...
import {
  BOARDROOM_ALLOWED_HOST,
  BOARDROOM_RSS_URL,
  BOARDROOM_SOURCE_ID,
  BoardroomRssFeedSchema,
...
function requireHttpsAndHost(url: string) {
  const u = new URL(url);
  if (u.protocol !== "https:") throw new Error("boardroom_network_blocked:non_https");
  if (u.hostname !== BOARDROOM_ALLOWED_HOST) throw new Error("boardroom_network_blocked:wrong_host");
  return u;
}
...
export function normalizeBoardroomCanonicalUrl(rawUrl: string): string {
...
export type BoardroomCollectionOutput =
  | {
      ok: true;
      source_id: typeof BOARDROOM_SOURCE_ID;
      feed: { feed_url: string; feed_title: string | null };
      items: BoardroomRssItem[];
...
export async function collectBoardroomRssV1(input: { now_iso: string; max_items: number }):
  Promise<BoardroomCollectionOutput> {
  try {
    const parsed = await fetchRssFeed(BOARDROOM_RSS_URL, {
      timeoutMs: 10_000,
    ...
    const items = parsed
      .map((it) => {
        const canonical_url = normalizeBoardroomCanonicalUrl(it.url);
        ...
        const rss_content_html = safeTrim(it.contentHtml ?? null, 50_000);
        ...
        return {
          ...
          excerpt,
          rss_content_html
        } satisfies BoardroomRssItem;
      })
      .sort((a, b) => a.canonical_url.localeCompare(b.canonical_url));

    const bounded = items.slice(0, Math.max(0, Math.floor(input.max_items)));
    const feed = BoardroomRssFeedSchema.parse({ feed_url: BOARDROOM_RSS_URL, feed_title: null, items: bounded });
    ...
    return { ok: false, source_id: BOARDROOM_SOURCE_ID, error: error instanceof Error ? error.message : String(error) };
  }
}

export function computeBoardroomSourceItemId(input: { canonical_url: string; guid: string | null }): string {
  return input.guid ? `guid:${input.guid}` : `url:${input.canonical_url}`;
}

export function computeBoardroomEvidenceReferenceId(input: { canonical_url: string }): string {
