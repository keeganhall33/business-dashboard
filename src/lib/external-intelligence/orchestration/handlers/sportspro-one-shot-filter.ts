export type SportsProCollectedItemBase = {
  canonical_url: string;
  guid: string | null;
};

export type SportsProOneShotFilter = {
...
export function normalizeSportsProOneShotFilter(input: null | SportsProOneShotFilter) {
 ...
 */
export function filterSportsProItemsForOneShot<T extends SportsProCollectedItemBase>(input: {
  items: T[];
  filter: ReturnType<typeof normalizeSportsProOneShotFilter>;
  ...
  for (const item of input.items) {
    ...
    const ok = (allowedEvidenceIds.size ? allowedEvidenceIds.has(evId) : false) || (allowedUrls.size ? allowedUrls.has(item.canonical_url) : false);
