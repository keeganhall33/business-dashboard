...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
      ...
      <dd className="mt-1 text-sm font-semibold text-slate-900">{humanize(value)}</dd>
    </div>
  );
}
...
function OpportunityCard({ item }: { item: PlanningReadinessOpportunityCardV1 }) {
  ...
  return (
    <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-3 rounded-2xl border border-slate-200 bg-[#ffffff] p-4">
        <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500">Strategic upside and economics</p>
        <p className="mt-1 text-sm font-semibold text-slate-900">{humanize(result.netStrategicEconomics)}</p>
        <p className="mt-2 text-xs leading-5 text-slate-600">Qualitative evidence only. No invented revenue, price, buyer, or certainty.</p>
      </div>

      <details className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
        <summary className="cursor-pointer text-sm font-semibold text-slate-900">Why now, evidence, and what would change</summary>
        <div className="mt-3 space-y-3 text-xs leading-5 text-slate-700">
          ...
          <div>
            <p className="font-semibold text-slate-900">What would change the recommendation</p>
          ...
          <p><span className="font-semibold text-slate-900">Evidence:</span> {result.evidenceRefs.join(" · ")}</p>
        </div>
      </details>
    </article>
  );
...
export function PlanningReadinessOpportunityPanelV1({
  ...
  return (
    <section aria-label="Opportunity planning readiness" data-testid="planning-readiness-opportunity-panel-v1" data-visual-mode="light" className="bg-[#f4f7fb] px-4 py-8 text-slate-950 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <header className="rounded-[2rem] border border-slate-200 bg-[#ffffff] p-5 shadow-sm md:p-7">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Opportunity intelligence · planning readiness</p>
          <div className="mt-3 grid gap-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-700 md:text-base">Read-only planning guidance combines runway, production fit, differentiation, and supported economics. It proposes timing without outreach, scheduling, or autonomous action.</p>
            </div>
