...
export function ConversionWatchPanel({ snapshot }: { snapshot?: WebsiteConversionSnapshot | null }) {
  ...
  const rangeLabel = formatWindow(snapshot);
  const ageHours = snapshot?.generatedAt ? (renderTimestamp - new Date(snapshot.generatedAt).getTime()) / 36e5 : null;
  ...
  const trackingSuspect = Boolean(
    ...
    (
      ...
      ((wooOrders ?? 0) > 0 && (addToCartEvents ?? 0) === 0 && (purchaseEvents ?? 0) === 0)
    )
  );
  ...
  const stateCopy = buildStateCopy({ state, sessions, wooOrders, gaPurchases, addToCartEvents, purchaseEvents });
...
function buildStateCopy(args: {
  ...
  const { state, sessions, wooOrders, gaPurchases, addToCartEvents, purchaseEvents } = args;
  if (state === "STALE") {
    return {
      badge: "Data stale",
      tone: "rose" as const,
  ...
  if (state === "TRACKING") {
    return {
      ...
      hint: purchaseEvents === 0 && addToCartEvents === 0 ? "GA4 add_to_cart and purchase events were missing entirely while Woo saw orders." : null
    };
  }
  if (state === "LOW") {
    return {
...
function formatWindow(snapshot?: WebsiteConversionSnapshot | null) {
  const start = snapshot?.wooCommerce?.windowStart;
  const end = snapshot?.wooCommerce?.windowEnd;
  if (start && end) return formatDateRangeLabel({ startDate: start, endDate: end });
  const rangeDays = snapshot?.wooCommerce?.rangeDays;
  return rangeDays ? `Last ${rangeDays} days` : null;
}
