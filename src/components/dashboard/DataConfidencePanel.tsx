...
export function DataConfidencePanel({ summary }: { summary: ConfidenceSummary }) {
  ...
  const trustedCount = summary.trustedSources.length;
  const watchCount = caveats.length;
  const unavailableCount = summary.insufficientSources.length;
  const topIssue = summary.topRisk;
  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-white/[0.02] p-5 sm:p-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <div className="mt-1 text-xs text-zinc-500">
            Trusted {trustedCount} • Watch {watchCount} • Unavailable {unavailableCount}
          </div>
        </div>
      ...
      <details className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4">
        <summary className="cursor-pointer select-none text-sm font-semibold text-zinc-200">Show full confidence breakdown</summary>
