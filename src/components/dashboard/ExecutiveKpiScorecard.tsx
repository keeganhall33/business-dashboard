import { HeaderMetric, RangePreset } from "@/lib/types/dashboard";
...
export function ExecutiveKpiScorecard({ metrics, range }: { metrics: HeaderMetric[]; range: { preset: RangePreset } }) {
  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-white/[0.02] p-6">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {selected.map((metric) => (
          <KpiCard key={metric.metricKey} metric={metric} rangePreset={range.preset} />
        ))}
      </div>
    </section>
  );
...
function KpiCard({ metric, rangePreset }: { metric: HeaderMetric; rangePreset: RangePreset }) {
  ...
  const currentUnavailable = metric.currentValue == null;
  const targetUnavailable = metric.targetValue == null;
  const assumesMonthlyTarget = metric.metricKey === "monthly_revenue" || metric.metricKey === "aov" || metric.metricKey === "conversion_rate";
  const rangeIsMonthly = rangePreset === "month_to_date" || rangePreset === "previous_month";
  const targetComparable = assumesMonthlyTarget ? rangeIsMonthly : true;
  ...
  return (
    <article className="flex flex-col rounded-2xl border border-white/5 bg-black/30 p-4 shadow-inner shadow-black/40">
      ...
      <div className="mt-3 text-3xl font-semibold text-white">{formatMetricValue(metric.currentValue, metric.unit)}</div>
      {targetUnavailable ? (
        <div className="mt-1 text-xs text-zinc-500">Target Unavailable</div>
      ) : targetComparable ? (
        <div className="mt-1 text-xs text-zinc-500">Target {metric.targetLabel ?? formatMetricValue(metric.targetValue, metric.unit)}</div>
      ) : (
        <div className="mt-1 text-xs text-zinc-500">Monthly target {metric.targetLabel ?? formatMetricValue(metric.targetValue, metric.unit)}</div>
      )}
