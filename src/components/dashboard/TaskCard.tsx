...
export function TaskCard({ task }: Props) {
  ...
  const [loggingDeliverable, setLoggingDeliverable] = useState(false);
  ...
  async function completeTask() {
    if (!deliverableSummary.trim()) {
      setError("Add a short deliverable summary first.");
      return;
    }
    setError(null);
    const res = await fetch(`/api/tasks/${task.id}/complete`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ resultSummary: deliverableSummary.trim() })
    });
    if (!res.ok) throw new Error(`Complete failed (${res.status})`);
  }

  const canLogDeliverable = !task.deliverableSummary && ["approved", "in_progress"].includes(task.status);
          ...
          {loggingDeliverable ? (
            <>
              <textarea
                ...
                placeholder="Summarize what shipped and link any proof"
                rows={4}
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="rounded-xl border border-emerald-500 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-200 hover:bg-emerald-500/20 disabled:opacity-50"
                >
                  Save deliverable
                </button>
                <button
                  ...
                  className="rounded-xl border border-zinc-700 px-3 py-2 text-sm text-zinc-100 hover:bg-zinc-900"
                >
                  Cancel
                </button>
              </div>
            ...
            <button
              onClick={() => setLoggingDeliverable(true)}
              className="rounded-xl border border-zinc-700 px-3 py-2 text-sm text-zinc-100 hover:bg-zinc-900"
            >
              Log deliverable
            </button>
