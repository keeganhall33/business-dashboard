        ...
        <div className="min-w-0 px-4 py-5 md:px-8 md:py-8">
          <section id="home" className="mx-auto max-w-4xl">
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-[minmax(0,1fr)_300px]">
              <div className="rounded-3xl border border-stone-200 bg-white p-4 shadow-sm md:p-5"><h3 className="text-sm font-semibold text-stone-950">What changed</h3><ul className="mt-3 space-y-2 text-sm leading-6 text-stone-700">{data.home.changed_since_last_review.map((item) => <li key={item}>{item}</li>)}</ul></div>
          ...
          <section className="mx-auto mt-8 grid max-w-6xl gap-3 md:grid-cols-2">
            <div className="rounded-3xl border border-stone-200 bg-white p-4"><h3 className="text-sm font-semibold text-stone-950">Mobile behavior</h3><ul className="mt-2 space-y-2 text-sm leading-6 text-stone-700">{data.responsive_behavior.mobile.map((item) => <li key={item}>{item}</li>)}</ul></div>
            <div className="rounded-3xl border border-stone-200 bg-white p-4"><h3 className="text-sm font-semibold text-stone-950">Desktop behavior</h3><ul className="mt-2 space-y-2 text-sm leading-6 text-stone-700">{data.responsive_behavior.desktop.map((item) => <li key={item}>{item}</li>)}</ul></div>
          </section>
        </div>
      </div>
    </main>
