...
export function KnowledgeHealthPanelV1({ view }: { view: KnowledgeHealthViewV1 }) {
  ...
  return (
    <main className="min-h-screen bg-[#f4f7fb] text-slate-950" aria-label="Knowledge Health workspace">
      <div className="mx-auto w-full max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        <header className="rounded-3xl border border-slate-200 bg-[#ffffff] p-5 shadow-sm sm:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Company brain integrity</p>
          ...
          <p className="mt-4 rounded-2xl border border-slate-200 bg-white p-4 text-sm leading-6 text-slate-700">{view.status_message}</p>
        ...
        <section className="mt-5" aria-label="Knowledge integrity priority queue">
          <div className="mb-3">
            ...
            <p className="mt-1 text-sm text-slate-600">Decision-corrupting conflicts appear before lower-impact housekeeping.</p>
          </div>
          {view.queue.length ? (
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-600">Only the supplied canonical data was checked. Missing coverage remains visible as UNKNOWN.</p>
            </div>
          )}
        </section>
      </div>
  ...
  return (
    <article className="rounded-3xl border border-slate-200 bg-[#ffffff] p-5 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">{item.affected}</p>
      ...
      <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
        <div><dt className="text-xs font-semibold text-slate-500">Evidence</dt><dd className="mt-1">{item.evidence_count}</dd></div>
        <div><dt className="text-xs font-semibold text-slate-500">Sources</dt><dd className="mt-1">{item.source_count}</dd></div>
        <div><dt className="text-xs font-semibold text-slate-500">Freshness</dt><dd className="mt-1">{item.freshness_state}</dd></div>
        <div><dt className="text-xs font-semibold text-slate-500">Next safe step</dt><dd className="mt-1">{item.next_step.replaceAll("_", " ")}</dd></div>
      </dl>
      <div className="mt-4 flex flex-wrap gap-2">
        {item.detail_href ? <a href={item.detail_href} className="rounded-full border border-slate-300 bg-white px-3 py-2 text-xs font-semibold">Open supported record</a> : null}
        <button type="button" disabled className="cursor-not-allowed rounded-full bg-slate-200 px-3 py-2 text-xs font-semibold text-slate-500">
