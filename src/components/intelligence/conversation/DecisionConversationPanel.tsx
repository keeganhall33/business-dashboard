...
export function DecisionConversationPanel({
  ...
  return (
    <section id={viewModel.id} aria-label="Conversational decision panel" className="border border-slate-200 bg-white p-4 shadow-sm md:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">{viewModel.eyebrow}</p>
          <h2 className="mt-2 text-xl font-semibold tracking-normal text-slate-950 md:text-2xl">{viewModel.title}</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-700">{viewModel.strategic_question}</p>
        </div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_340px]">
        <div className="min-w-0 space-y-4">
          <article className="border border-slate-200 bg-slate-50 p-4">
            ...
            <h3 className="mt-3 text-base font-semibold text-slate-950">{viewModel.recommendation.title}</h3>
            <p className="mt-2 text-sm leading-6 text-slate-700">{viewModel.recommendation.summary}</p>
            <p className="mt-3 text-sm font-semibold text-slate-950">{viewModel.recommendation.action}</p>
          </article>

          <form className="border border-slate-200 bg-white p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <label htmlFor={`${viewModel.id}-input`} className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">
            ...
            <div className="mt-3 flex flex-col gap-2 sm:flex-row">
              <input
                ...
                className="min-w-0 flex-1 border border-slate-300 bg-slate-50 px-3 py-2 text-sm text-slate-950 outline-none"
          ...
          <article className="border border-slate-200 bg-slate-50 p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Grounded answer</p>
            <p className="mt-2 text-sm leading-6 text-slate-900">{viewModel.answer.written_answer}</p>
            <p className="mt-3 text-sm leading-6 text-slate-700"><strong className="text-slate-950">Spoken:</strong> {viewModel.answer.spoken_answer}</p>
          </article>
        </div>

        <aside className="space-y-4">
          <section className="border border-slate-200 bg-white p-4">
            <h3 className="text-sm font-semibold text-slate-950">Suggested next questions</h3>
            <div className="mt-3 grid gap-2">
              {viewModel.suggested_questions.map((question) => (
                <button key={question.id} type="button" className="border border-slate-200 bg-slate-50 px-3 py-2 text-left text-sm font-semibold text-slate-800">
                  {question.label}
                </button>
              ))}
            </div>
          ...
          <section className="border border-slate-200 bg-white p-4">
            ...
            <div className="mt-3 space-y-2">
              {viewModel.answer.evidence_refs.map((item) => (
                <div key={item.id} className="border border-slate-200 bg-slate-50 p-3 text-sm">
                  ...
                  <p className="mt-2 leading-6 text-slate-700">{item.provenance} | {item.source}</p>
          ...
          <section className="border border-slate-200 bg-white p-4">
            <h3 className="text-sm font-semibold text-slate-950">Assumptions / unknowns</h3>
            <div className="mt-3 space-y-2">
              {viewModel.answer.assumptions.map((item) => (
                <div key={item.id} className="border border-slate-200 bg-slate-50 p-3 text-sm">
                  ...
                  <p className="mt-2 leading-6 text-slate-700">{item.detail}</p>
                </div>
              ))}
