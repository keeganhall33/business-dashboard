...
export function IonosCommunicationAttentionCard({
  ...
  if (inbox == null) {
    return (
      ...
      >
        <div className="rounded-3xl border border-slate-200 bg-[#ffffff] p-4 shadow-sm md:p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Relationship intelligence</p>
          <div className="mt-2 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="text-xl font-semibold tracking-normal text-slate-950">Communication attention</h2>
            ...
            <a
              href="/relationships"
              className="rounded-full border border-slate-300 bg-white px-4 py-2 text-center text-sm font-semibold text-slate-800 shadow-sm"
            >
              Open Relationships
            </a>
          </div>
    ...
    >
      <div className="rounded-3xl border border-slate-200 bg-[#ffffff] p-4 shadow-sm md:p-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Relationship intelligence</p>
            <h2 className="mt-1 text-xl font-semibold tracking-normal text-slate-950">Communication attention</h2>
            <p className="mt-1 text-sm leading-6 text-slate-600">
              Canonical priority order only. Suggested next moves remain preparation-only until separately approved.
            </p>
          </div>
          <a href="/relationships" className="text-sm font-semibold text-slate-800 underline underline-offset-4">
            Open Relationships
          </a>
        </div>
        ...
        {items.length === 0 ? (
          ...
          <div className="mt-4 grid gap-3 lg:grid-cols-3">
            {items.map((item) => {
              ...
              return (
                <article key={item.id} data-attention-id={item.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                  <div className="flex flex-wrap gap-2 text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-600">
                    <span className="rounded-full border border-slate-200 px-2 py-1">{item.truthState}</span>
                    <span className="rounded-full border border-slate-200 px-2 py-1">{item.freshnessState}</span>
                  </div>
                  <p className="mt-3 text-xs font-semibold text-slate-500">{mailboxContext(item)}</p>
                  <dl className="mt-3 space-y-3 text-sm leading-6 text-slate-700">
                    <div>
                      <dt className="font-semibold text-slate-950">What happened</dt>
                    ...
                    <div>
                      <dt className="font-semibold text-slate-950">Next safe move</dt>
                      <dd>{item.nextMove}</dd>
                    </div>
                  </dl>
                  ...
                  <a href={href} className="mt-3 inline-block text-sm font-semibold text-slate-900 underline underline-offset-4">
