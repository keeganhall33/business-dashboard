...
const statusTone: Record<CreativeVisualizationConceptV1["VISUALIZATION_STATUS"], string> = {
  READY_FOR_REVIEW: "border-slate-300 bg-white text-slate-800",
...
export function CreativeVisualizationPanel({ comparisonSet }: { comparisonSet: CreativeVisualizationComparisonSetV1 }) {
  return (
    <section className="mt-6 rounded-3xl border border-slate-200 bg-[#ffffff] p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="max-w-3xl">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Creative visualization</p>
          <h2 className="mt-2 text-2xl font-semibold tracking-normal">Visualize this recommendation</h2>
          <p className="mt-3 text-sm leading-6 text-slate-700">{comparisonSet.epistemic_guardrail}</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700">
          <div className="font-semibold text-slate-950">{comparisonSet.request.concept_count} concept studies</div>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
        <article className="rounded-2xl border border-slate-200 bg-white p-4">
          <h3 className="text-sm font-semibold uppercase tracking-[0.16em] text-slate-500">Controlled-variable comparison</h3>
          <div className="mt-3 grid gap-3 md:grid-cols-3">
            {comparisonSet.comparison_axes.map((axis) => (
              <div key={axis} className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-800">
                {axis}
              </div>
            ))}
          </div>
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-4">
          <h3 className="text-sm font-semibold uppercase tracking-[0.16em] text-slate-500">Evidence boundary</h3>
...
function ConceptCard({ concept }: { concept: CreativeVisualizationConceptV1 }) {
  return (
    <article className="flex min-h-[560px] flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">{concept.CONCEPT_ID}</p>
      ...
      <div className="mt-4 aspect-[4/3] rounded-xl border border-slate-200 bg-[linear-gradient(135deg,#f4f7fb,#ddd5c8)] p-4">
        <div className="h-full rounded-lg border border-slate-300 bg-white/55 p-3 text-xs leading-5 text-slate-700">
          <div className="font-semibold text-slate-950">{concept["DIMENSIONS/ASPECT"]}</div>
      ...
      <div className="mt-4">
        <h4 className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Controlled variables</h4>
        <ul className="mt-2 space-y-2 text-sm leading-6 text-slate-800">
          {concept.CONTROLLED_VARIABLES.map((item) => (
            <li key={`${concept.CONCEPT_ID}-${item.variable}`} className="rounded-xl bg-slate-50 p-2">
      ...
      <div className="mt-auto pt-4">
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm leading-6 text-slate-800">
        ...
        <p className="mt-3 text-xs leading-5 text-slate-500">{concept.WHAT_THIS_VISUAL_DOES_NOT_PROVE.join(" ")}</p>
      </div>
    </article>
  );
}
