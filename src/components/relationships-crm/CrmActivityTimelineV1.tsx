...
function ActivityItem({ item }: { item: CrmActivityTimelineItemV1 }) {
  ...
  return (
    <article className="grid gap-3 border-t border-slate-200 py-4 first:border-t-0 md:grid-cols-[10rem_minmax(0,1fr)]" data-activity-kind={item.kind}>
      <div>
        <p className="text-xs font-semibold text-slate-500">{displayTimestamp(item.timestamp)}</p>
        <p className="mt-1 text-xs text-slate-500">{KIND_LABELS[item.kind]}</p>
      </div>
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {item.direction ? (
            <span className="rounded-full border border-slate-200 bg-white px-2 py-1 text-[10px] font-semibold text-slate-700">{item.direction}</span>
          ) : null}
          <span className="rounded-full border border-slate-200 bg-white px-2 py-1 text-[10px] font-semibold text-slate-700">{item.truthState}</span>
          <span className="rounded-full border border-slate-200 bg-white px-2 py-1 text-[10px] font-semibold text-slate-700">{item.freshnessState}</span>
        </div>
        ...
        <div className="mt-3 flex flex-wrap gap-2 text-xs font-semibold">
          {relationshipHref ? <a className="text-slate-800 underline decoration-slate-300 underline-offset-4" href={relationshipHref}>Person record</a> : null}
          {companyHref ? <a className="text-slate-800 underline decoration-slate-300 underline-offset-4" href={companyHref}>Company record</a> : null}
          {item.opportunityIds.map((opportunityId) => (
            <span key={opportunityId} className="text-slate-500">Opportunity: {opportunityId}</span>
          ))}
        </div>
        <details className="mt-3 text-xs text-slate-600">
          <summary className="cursor-pointer font-semibold text-slate-700">Evidence details</summary>
...
export function CrmActivityTimelineV1({ timeline }: { timeline: CrmActivityTimelineViewV1 | null }) {
  return (
    <main className="min-h-screen bg-[#f4f7fb] px-4 py-6 text-slate-950 sm:px-6 lg:px-8" data-testid="crm-activity-timeline-v1">
        ...
        <header className="rounded-[2rem] border border-slate-200 bg-[#ffffff] p-5 shadow-sm md:p-7">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <nav aria-label="CRM activity navigation" className="flex flex-wrap gap-2">
              ...
              <a href="/relationships/companies" className="rounded-full border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-800">Companies</a>
            </nav>
          </div>
        </header>

        {!timeline ? (
          <section className="mt-5 rounded-3xl border border-dashed border-slate-300 bg-white p-6" aria-labelledby="crm-activity-unavailable-title">
          ...
          <section className="mt-5 rounded-3xl border border-dashed border-slate-300 bg-white p-6">
            ...
            <p className="mt-1 text-sm text-slate-600">Canonical activity evidence supplied no timeline items.</p>
          </section>
        ) : (
          <>
            ...
            <section className="mt-5 rounded-3xl border border-slate-200 bg-white px-5 shadow-sm" aria-label="Chronological relationship activity">
