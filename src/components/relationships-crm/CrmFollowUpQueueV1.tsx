...
function QueueItem({ item }: { item: CanonicalRelationshipFollowUpQueueItemV1 }) {
  return (
    <article
      className="border-t border-stone-200 py-5 first:border-t-0"
    ...
    >
        ...
        <div>
          <div className="flex flex-wrap gap-2">
            {item.queueClasses.map((queueClass) => (
              <span key={queueClass} className="rounded-full border border-stone-200 bg-stone-50 px-2 py-1 text-[10px] font-semibold text-stone-700">
                {QUEUE_LABELS[queueClass]}
              </span>
            ))}
          </div>
          <h2 className="mt-3 text-lg font-semibold text-stone-950">Contact {item.contactId}</h2>
          <p className="mt-1 text-sm text-stone-600">Suggested move: {item.suggestedMove.replaceAll("_", " ")}</p>
        </div>
        <div className="grid grid-cols-2 gap-x-5 gap-y-1 text-xs text-stone-600 lg:text-right">
          <span>Priority</span><strong className="text-stone-900">{item.priority}</strong>
          <span>Due</span><strong className="text-stone-900">{displayTimestamp(item.dueAt)}</strong>
          <span>Last interaction</span><strong className="text-stone-900">{displayTimestamp(item.lastMeaningfulInteractionAt)}</strong>
          <span>Truth</span><strong className="text-stone-900">{item.truthState}</strong>
          <span>Freshness</span><strong className="text-stone-900">{item.freshnessState}</strong>
        </div>
      </div>
      ...
      <div className="mt-3 flex flex-wrap gap-3 text-xs font-semibold">
        <a className="text-stone-800 underline decoration-stone-300 underline-offset-4" href={`/relationships/people/${encodeURIComponent(item.contactId)}`}>
          Person record
        </a>
      ...
      <details className="mt-3 text-xs text-stone-600">
        <summary className="cursor-pointer font-semibold text-stone-700">Evidence and identity</summary>
...
export function CrmFollowUpQueueV1({ queue }: { queue: CanonicalRelationshipFollowUpQueueResultV1 | null }) {
  ...
  return (
    <main className="min-h-screen bg-[#f8f4ec] px-4 py-6 text-stone-950 sm:px-6 lg:px-8" data-testid="crm-follow-up-queue-v1">
      <div className="mx-auto max-w-[1400px]">
        <header className="rounded-[2rem] border border-stone-200 bg-[#fffdf8] p-5 shadow-sm md:p-7">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <nav aria-label="CRM follow-up navigation" className="flex flex-wrap gap-2">
              ...
              <a href="/relationships/activity" className="rounded-full border border-stone-300 bg-white px-4 py-2 text-sm font-semibold text-stone-800">Activity</a>
        ...
        {!queue ? (
          <section className="mt-5 rounded-3xl border border-dashed border-stone-300 bg-white p-6" aria-labelledby="crm-follow-ups-unavailable-title">
          ...
          <>
            <section className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4" aria-label="Follow-up queue counts">
              {QUEUE_ORDER.map((queueClass) => (
                <div key={queueClass} className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm">
                  <p className="text-xs font-semibold uppercase tracking-[0.1em] text-stone-500">{QUEUE_LABELS[queueClass]}</p>
                  <p className="mt-1 text-2xl font-semibold tabular-nums text-stone-950">{queue.counts[queueClass]}</p>
                </div>
              ))}
            </section>
            ...
            {items.length === 0 ? (
              <section className="mt-5 rounded-3xl border border-dashed border-stone-300 bg-white p-6">
                ...
                <p className="mt-1 text-sm text-stone-600">Canonical queue evidence supplied no actionable or verification-required records.</p>
              </section>
            ) : (
              <section className="mt-5 rounded-3xl border border-stone-200 bg-white px-5 shadow-sm" aria-label="Prioritized relationship follow-ups">
