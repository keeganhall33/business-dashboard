  ...
  return (
    <div className="space-y-6">
      <header className="space-y-1">
        <h1 className="text-2xl font-semibold text-slate-950">Action</h1>
        <p className="text-sm text-slate-600">Details, evidence, measurement, and audit (no execution).</p>
      </header>
      ...
      <VerticalSliceCard title={String(action.title ?? "")} subtitle={`${String(action.category ?? "")} • ${String(action.channel ?? "")}`}>
        ...
        {action.evidence_snapshot_hash ? (
          <div className="mt-3 text-xs text-slate-500">Evidence hash: {String(action.evidence_snapshot_hash).slice(0, 20)}…</div>
        ) : null}
      </VerticalSliceCard>
      ...
      <VerticalSliceCard title="Prepared assets" subtitle="Drafts + previews. Editing is allowed only at the readiness boundary.">
        <pre className="max-h-[320px] overflow-auto rounded-2xl border border-slate-200 bg-slate-50 p-4 text-xs text-slate-800">
      ...
      <VerticalSliceCard title="Measurement plan" subtitle="Required before approval.">
        <pre className="max-h-[260px] overflow-auto rounded-2xl border border-slate-200 bg-slate-50 p-4 text-xs text-slate-800">
      ...
      <VerticalSliceCard title="Audit trail" subtitle="Every transition must be audited with an idempotency key.">
        <div className="space-y-2">
          {audit.length ? (
            audit.map((e, idx) => (
              <div key={String(e.id ?? idx)} className="rounded-2xl border border-slate-200 bg-white p-4">
                <div className="text-xs text-slate-600">{String(e.created_at ?? "")} • {String(e.actor ?? "")} • {String(e.event_type ?? "")}</div>
                <div className="mt-1 text-xs text-slate-500">{String(e.from_status ?? "")} → {String(e.to_status ?? "")}</div>
                {e.idempotency_key ? <div className="mt-1 text-[11px] text-slate-500">idempotency: {String(e.idempotency_key)}</div> : null}
                {e.note ? <div className="mt-2 text-xs text-slate-800">{String(e.note)}</div> : null}
              </div>
            ))
          ) : (
            <div className="text-sm text-slate-500">No audit events.</div>
          )}
        </div>
      </VerticalSliceCard>
    </div>
