...
function SpecialistsWorkspace({ capabilities }: { capabilities: SpecialistCapabilityStatusV1[] }) {
  ...
  return (
    <main className="min-h-screen bg-[#f4f7fb] text-slate-950" aria-label="Specialist intelligence workspace">
      <div className="mx-auto w-full max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        <header className="rounded-3xl border border-slate-200 bg-[#ffffff] p-5 shadow-sm sm:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Production truth boundary</p>
              <h1 className="mt-2 text-3xl font-semibold tracking-tight text-slate-950 md:text-4xl">Specialists</h1>
        ...
        <section className="mt-5" aria-label="Specialist capability status">
          <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h2 className="text-xl font-semibold text-slate-950">Capability status</h2>
              <p className="mt-1 text-sm leading-6 text-slate-600">What can be trusted now, what is missing, and the next safe evidence step.</p>
            </div>
            <a href="/data-evidence" className="inline-flex w-fit rounded-full border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-800 shadow-sm">
              Open Data &amp; Evidence
            </a>
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            {capabilities.map((capability) => (
              <article key={capability.id} className="rounded-3xl border border-slate-200 bg-[#ffffff] p-5 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">{capability.availability}</p>
                    <h3 className="mt-1 text-xl font-semibold text-slate-950">{capability.title}</h3>
                  </div>
                ...
                {capability.detail_href ? (
                  ...
                  <div className="mt-5 rounded-2xl border border-dashed border-slate-300 bg-white p-3 text-xs leading-5 text-slate-600">
        ...
        <section className="mt-5 grid gap-4 md:grid-cols-2" aria-label="Specialist operating boundary">
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">What this prevents</p>
            <h2 className="mt-1 text-lg font-semibold text-slate-950">Fixture conclusions masquerading as live intelligence</h2>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Where to work meanwhile</p>
            <h2 className="mt-1 text-lg font-semibold text-slate-950">Use production-backed workspaces directly</h2>
            <div className="mt-3 flex flex-wrap gap-2">
              <a href="/relationships" className="rounded-full border border-slate-300 bg-white px-3 py-1.5 text-xs font-semibold text-slate-800">Relationships</a>
              <a href="/opportunities-actions" className="rounded-full border border-slate-300 bg-white px-3 py-1.5 text-xs font-semibold text-slate-800">Opportunities &amp; Actions</a>
              <a href="/dashboard" className="rounded-full border border-slate-300 bg-white px-3 py-1.5 text-xs font-semibold text-slate-800">Executive Home</a>
            </div>
          </div>
        </section>
      </div>
...
function TruthBoundaryMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4">
      ...
      <div className="mt-2 text-sm font-semibold leading-5 text-slate-950">{value}</div>
    </div>
  );
}
