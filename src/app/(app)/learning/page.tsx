...
export default async function LearningPage() {
  ...
  return (
    ...
    >
      <section className="bg-[#f4f7fb] px-4 pt-6 sm:px-6 lg:px-8" aria-label="Learning production status">
        <div className="mx-auto max-w-7xl rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-full border border-slate-300 bg-slate-50 px-2 py-1 text-[10px] font-semibold text-slate-800">
              {feed.capability.replaceAll("_", " ")}
            </span>
            <span className="text-xs text-slate-600">{feed.reason}</span>
          </div>
      ...
      {feed.status === "AVAILABLE" && feed.records.length > 0 ? (
        <section className="bg-[#f4f7fb] px-4 pb-8 sm:px-6 lg:px-8" aria-label="Canonical learning traceability">
              ...
              <details key={record.id} className="rounded-2xl border border-slate-200 bg-white p-4">
                ...
                <dl className="mt-3 grid gap-3 text-sm sm:grid-cols-2">
                  <div>
                    ...
                    <dd className="mt-1 text-slate-800">{record.recommendationId ?? "UNKNOWN"}</dd>
                  </div>
                  <div>
                    <dt className="font-semibold text-slate-500">Success metric</dt>
                    <dd className="mt-1 text-slate-800">{record.successMetric ?? "UNKNOWN"}</dd>
                  </div>
                  <div>
                    <dt className="font-semibold text-slate-500">Key assumptions</dt>
                    <dd className="mt-1 text-slate-800">
                      {record.keyAssumptions.length ? record.keyAssumptions.join(" · ") : "UNKNOWN"}
                    </dd>
                  </div>
                  <div>
                    <dt className="font-semibold text-slate-500">Confounders / unknowns</dt>
                    <dd className="mt-1 text-slate-800">
