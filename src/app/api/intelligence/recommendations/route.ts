  ...
  try {
    ...
    if (!comparisonRange) {
      const payload: RecommendationsResponse = {
        ...
        recommendations: [],
        guardrailsTriggered: ["Invalid comparison range"],
        warnings: []
      };
      return ok(payload);
    }
    ...
    const payload = buildRecommendationsFromExplanation({ explanation, missingSources });
    return ok(payload);
  } catch (error) {
    return serverError("Failed to build recommendations", { message: error instanceof Error ? error.message : String(error) });
  }
}
